...
bool PLYLoader::ParsePLYHeader(std::ifstream &file, PLYHeader &header) {
    ...
    while (std::getline(file, line)) {
        ...
        if (keyword == "format") {
            ...
            iss >> format >> version;
            header.is_binary = (format == "binary_little_endian" || format == "binary_big_endian");
    ...
    if (header.is_binary) {
        ...
        for (int i = 0; i < header.vertex_count; i++) {
            if (!file.read(point_data.data(), header.properties.size() * sizeof(float))) {
                std::cerr << "Warning: Only read " << i << " of " << header.vertex_count
                          << " points" << std::endl;
                break;
            }
            ...
            if (std::isfinite(x) && std::isfinite(y) && std::isfinite(z) &&
                norm > config_.preprocess_.min_range && norm < config_.preprocess_.max_range) {
        ...
        while (points.size() < header.vertex_count && std::getline(file, line)) {
            ...
            const double norm =
                Eigen::Vector3d(values[header.x_idx], values[header.y_idx], values[header.z_idx])
                    .norm();
            if (norm > config_.preprocess_.min_range && norm < config_.preprocess_.max_range) {
                points.emplace_back(values[header.x_idx], values[header.y_idx],
